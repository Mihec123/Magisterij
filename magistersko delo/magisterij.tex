...
Pri konstrukciji Weilovega parjenja bodo pomembno vlogo igrali t.\ i.\ delitelji. V tem poglavju bomo navedli nekaj definicij in lastnosti, ki jih bomo kasneje potrebovali.
...
Obstaja taka funkcija $u_P$, imenovana \emph{uniformizer} v točki $P$ z lastnostjo $u_P(P) = 0$. Prav tako velja, da lahko vsako funkcijo $f(x,y)$ zapišemo kot
$$f = u^r_Pg, \ r\in \Z,\ g(P) \neq 0,\infty.$$
...
\section{MOV}
MOV napad s pomočjo Weilovega parjenja pretvori problem diskretnega logaritma iz $E(\F_{q})$ v problem diskretnega logaritma nad $\F^{\times}_{q^m}$. Na ta način se izognemo težji strukturi grupe. Nov problem diskretnega logaritma pa lahko sedaj rešimo z različnimi napadi, med drugim tudi z napadom Index-Calculus \ref{IndexCalc}. MOV napad deluje če velikost polja $\F_{q^m}$ ni dosti večja od velikosti polja $\F_{q}$. Postopek napada sledi poteku dokaza naslednje trditve.
