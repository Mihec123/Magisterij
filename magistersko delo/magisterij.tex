...
\subsection*{Kubične krivulje v kriptografiji}
%\noindent Kubične krivulje v kriptografiji

\vspace{10mm}
\noindent naj predstavi strukturo Abelove grupe na ravninskih kubičnih krivuljah definiranimi nad končnimi polji. 
Delo naj obravnava uporabo eliptičnih krivulj v kriptografiji. 
...
\vfill
\begin{center}
\textbf{Elliptic curves in cryptography} \\[3mm] % prevod slovenskega naslova dela
\textsc{Abstract}\\[2mm]
\end{center}
...
V tem magisterskem delu bomo (enako kot v literaturi) gladkim kubičnim krivuljam pogosto rekli eliptične krivulje, saj so nad poljem $\C$ vse definicije ekvivalentne.
Najprej  bomo spoznali osnovne definicije in izreke o kubičnih krivuljah definiranimi nad končnimi polji. Za tem pa se bomo posvetili problemu diskretnega logaritma nad eliptičnimi krivuljami. Nato bomo spoznali nekaj napadov na eliptične krivulje, ter analizirali primerno izbiro krivulj za kriptografske namene. Pri tem bomo uvedli tudi parjenje nad eliptičnimi krivuljami in spoznali učinkovit algoritem za izračun le tega. Za vse skupaj pa bomo napisali tudi programsko kodo. Del kode bo napisan v programskem jeziku Python. Nekaj kode pa bomo zaradi lažje implementacije, prihranili si bomo implementacijo različnih algebraičnih objektov, napisali v prosto dostopnem programskem okolju SAGE~\cite{sagemath}.
...
\label{tocke_na_krivulji}
Podpoglavji~\ref{tocke_na_krivulji} in~\ref{struktura_grupe} sta povzeti po~\cite{Avsec2017}.
\begin{definicija}
...
V algoritmih bomo velikokrat potrebovali vsaj eno točko na krivulji. S pomočjo algoritma Tonelli-Shanks~\cite{Tonelli1891} bomo točko na krivulji lahko učinkovito poiskali. Naj bo $p$ modul po katerem računamo.
Če privzamemo, da lahko naključno izberemo število $0 \leq x < p$, potem nas zanima, kako učinkovito izberemo točko $(x,y)$ na krivulji podani z enačbo
...
\begin{trditev}
Naj bo $p > 2$ praštevilo. Če je $n \in \F_p$ kvadratični ostanek po modulu $p$, potem nam Tonelli-Shanks algoritem vrne $r \in \F_p$, tako da velja $n = r^2$.
...
\State Najdi tak $z \in F_p$, da $z$ ni kvadratični ostanek
...
\begin{opomba}
V algoritmu~\ref{alg:Tonelli} lahko prvi korak izvedemo tako, da preiskušamo naključne $z\in \F_p$, dokler ne velja $z^{(p-1)/2} = -1$. To pa po Eulerjevem kriteriju~\cite{Eulerkriterij} pomeni, da $z$ ni kvadratični ostanek po modulu $p$.
\end{opomba}
...
\begin{primer}
Če vzamemo endomorfizem iz prejšnjega primera $\alpha(P) = 2P,$ po formuli iz leme~\ref{sestevanje} dobimo funkcijo
...
To pa ravno pomeni, da točke oblike $(x^q,y^q)$ ležijo na krivulji podani z enačbo~\eqref{enc:razWeier}.
...
vsebujeta vsaka po $q$ elementov in je $\F_q$ vsebovana v drugi množici, sledi da sta enaki.
...
To pa ravno pomeni, da se lahko vsak tak homomorfizem $\alpha$ predstavi z matriko oblike
...
\begin{proof}[Dokaz trditve~\ref{trd:3.16}]
Naj bo $n \in \Z$, število ki ni deljivo s karakteristiko polja. Predstavimo $\alpha,\beta$ z matrikama $\alpha_n,\beta_n$, glede na neko izbrano bazo $E[n]$.
Velja
\begin{align}
...
\begin{lema}
\label{lema:hasse}
Naj bosta $r,s \in \Z$ tuji števili. Potem obstaja število $a$, za katerega velja
...
\begin{lema}
\label{lema:3.3}
Polinom $\gamma_{n}$ je element $\Z[x,y^2,a,b]$ za vse lihe $n$.  Za sode $n$ pa je $\gamma_{n}$ element  $2y\Z[x,y^2,a,b]$.
...
Podobno kot pri polinomih $\gamma_m$, lahko tudi za $\phi_m$ in $\omega_m$ formuliramo lemo.

\begin{lema}
Polinom $\phi_{n}$ je element $\Z[x,y^2,a,b]$, za vse $n$. Če je $n$ lih, potem je $\omega_{n}$ element v $y\Z[x,y^2,a,b]$. Za sode $n$ pa je $\omega_{n}$ element v $\Z[x,y^2,a,b]$.

\end{lema}

\begin{proof}
Če je $n$ lih, potem sta $\phi_{n+1}$ in $\phi_{n-1}$ po lemi~\ref{lema:3.3} v $y\Z[x,y^2,a,b]$. To pomeni, da je njun produkt v $\Z[x,y^2,a,b]$. Od tod takoj sledi, da je $\phi_n$ element $\Z[x,y^2,a,b]$. V primeru, ko je $n$ sodo število, dokaz poteka podobno. Prav tako na podoben način pokažemo $\omega_n \in y^{-1}\Z[x,y^2,a,b]$ v primeru, ko je $n$ liho število. Za sode $n$ pa je potrebno malo več dela.
Z indukcijo lahko najprej dokažemo
$$\phi_n \equiv (x^2+a)^{(n^2-1)/4} \MOD{2}, \text{ ko je } n \text{ liho število}$$
in
$$(2y)^{-1}\phi_n \equiv (\frac{n}{2})(x^2+a)^{(n^2-4)/4} \MOD{2}, \text{ ko je } n \text{ sodo število}.$$

S pomočjo tega pa izračunamo
\begin{align}
\omega_n &{}= (4y)^{-1}(\gamma_{m+2}\gamma^2_{m-1}-\gamma_{m-2}\gamma^2_{m+1}) \nonumber \\
 &{}\equiv  (4y)^{-1}(2y\frac{n+2}{2}(x^2+A)^{\frac{(n+2)^2-1}{4} + \frac{(n-1)^2-1}{2}} \nonumber \\
&{} - 2y\frac{n-2}{2}(x^2+A)^{\frac{(n-2)^2-1}{4} + \frac{(n+1)^2-1}{2}}) \MOD{2}\nonumber \\
&{}\equiv (4y)^{-1}2y(x^2+A)^{\frac{3n^2}{4}}\left(\frac{n+2}{2}-\frac{n-2}{2}\right) \MOD{2}\nonumber \\
&{}\equiv (x^2+A)^{\frac{3n^2}{4}} \MOD{2}\nonumber
\end{align}
To pa pomeni, da je tudi za sode $n$, $\omega_n$ element $\Z[x,y^2,a,b]$.
\end{proof}

\begin{opomba}
Če bi pri dokazu, da je $\omega_{n}$ element v $\Z[x,y^2,a,b]$, za sode $n$, uporabili zgolj lemo~\ref{lema:3.3}, bi lahko dokazali le $\omega_n \in \frac{1}{2}\Z[x,y^2,a,b]$.
\end{opomba}
...
\begin{opomba}
Polinoma $\gamma_n$ ni vedno možno predstaviti kot polinom v spremenljivki $x$, saj je potenca $y$ odvisna od parnosti $n$. Vseeno pa velja, da lahko $\gamma^2_n$ vedno zapišemo kot polinom spremenljivke $x$.
...
gladkimi kubičnimi krivuljami, kateremu bomo posvetili nadaljna poglavja.
...
Po prvi točki izreka~\ref{trd-WeilPar} velja $$e_n(T_1,dT_2) = e_n(T_1,T_2)^d=1.$$ Prav tako velja $e_n(T_2,dT_2) = e_n(T_2,T_2)^d = 1 $. Naj bo $S \in E[n]$, potem iz $S = aT_1+bT_2$ za neka $a,b \in \N$, s ponovno uporabo izreka~\ref{trd-WeilPar} dobimo
$$e_n(S,dT_2)= e_n(T_1,dT_2)^ae_n(T_2,dT_2)^b = 1. $$
Ker to velja za vsak $S$, iz druge točke izreka~\ref{trd-WeilPar} sledi, da je $dT_2 = \infty$. To pa je možno le, če $n|d$, kar pomeni, da je $n \leq d$.
\end{proof}
...
Po izreku~\ref{trd-WeilPar} velja
$$\zeta = e_n(T_1,T_2) = e_n(\rho T_1,\rho T_2) = \rho(e_n(T_1,T_2)) = \rho(\zeta).$$

Osnovni izrek Galoisove teorije~\cite{milneFT} pa pravi, če je $x \in \overline{K}$ fiksen za vse avtomorfizme $\rho$, ki delujejo kot identiteta na elementih $K$, potem je $x\in K$. Torej je $\zeta \in K$. Ker pa je $\zeta$ primitivni koren enote, od tod sledi $\mu_n \subset K$.

\end{proof}

Pred dokazom izreka~\ref{trd-WeilPar} bomo potrebovali še eno trditev.
\begin{trditev}[\protect{\cite{Washington2008}, Trditev 9.34}]
\label{trd:9.34}

Naj bo $E$ eliptična krivulja nad poljem $K$. Naj bo $f$ funkcija, ki elementu iz $E$ priredi element v $\overline{K} \cup \{ \infty \}$, ter naj bo $n \in \N$ število, ki ni deljivo s karakteristiko $K$. Če velja $f(P+T) = f(P)$ za vse $P \in E(\overline{K})$ in $T \in E[n]$, potem obstaja funkcija $h$ na $E$, za katero velja
$$f(P) = h(nP), \ \text{za vse } P.$$

\end{trditev}

\begin{proof}[Dokaz izreka~\ref{trd-WeilPar}]
%%%%%%%%%%%%
%POMEMBNO
...
Označimo s $f \circ n$ funkcijo, ki točko pomnoži z $n$ in nato na njenem rezultatu izračuna $f$. Točke $P = T' + R$, kjer je $R\in E[n]$, so točke za katere velja $nP=T$. Iz~\ref{enacba-Weil} in definicije preslikav med delitelji~\cite[str.~29]{Silverman2009}
sledi

$$\Div{f \circ n} = n \left( \sum_{R \in E[n]}[T' + R] \right) - n \left( \sum_{R \in E[n]}[R] \right) = \Div{g^n}.$$
...
Ta definicija je dobra, ker je $g$ zaradi lastnosti delitelja določen do skalarja natančno, torej je definicija neodvisna od izbire $g$. Prav tako pa je definicija neodvisna od izbire $P$, vendar je obrazložitev bolj zahtevna in je ne bomo navedli. Lahko jo najdemo v~\cite{Silverman2009}.

Sedaj, ko smo uspešno definirali Weilovo parjenje, moramo preveriti, da zanj veljajo lastnosti $1$-$6$.
\begin{enumerate}
\item Ker je definicija neodvisna od izbire točke $P$, lahko uporabimo $P$ in $P+S_1$. Potem je
\begin{align}
...
Predpostavimo da so $T_1,T_2,T_3 \in E[n]$ z lastnostjo $T_1 +T_2 = T_3.$ Označimo z $f_i,g_i$ funkcije, ki spadajo k definicijam $e_n(S,T_i)$.
...
obstaja funkcija $h$, za katero velja $g = h \circ [n]$. Od tod sledi 
...
kar dokaže, da je $\Div{h} = [T] - [\infty]$. Po izreku~\ref{izrek 11.2} pa od tod sledi $T = \infty$. S tem smo dokazali eno polovico točke $2$, druga polovica pa avtomatično sledi iz tega ter uporabe točke $4$.

\item Označimo s $\tau_{jT}$ točko $jT$. V tem primeru $f \circ \tau_{jT}$ označuje funkcijo \newline $P \mapsto f(P+jT)$. Delitelj te funkcije je torej $n[T-jT]-n[-jT]$. Zaradi tega velja
...
Torej lahko namesto točke $P$ vanjo vstavimo točko $P+T'$ in dobimo
...
S tem smo dokazali, da je $$e_n(T,S) = e_n(S,T)^{-1}.$$

\item Naj bo $\rho$ avtomorfizem na $\overline{K}$, za katerega velja, da je $\rho$ identiteta na koeficientih $E$. Če z $\rho$ delujemo na vsakem koraku konstrukcije Weilovega parjenja, dobimo
$$\Div{f^{\rho}} = n[\rho T] - n[\infty],$$
ter podobno za $g^{\rho}$. Tu $f^{\rho},g^{\rho}$ označujeta funkciji, ki ju dobimo tako, da $\rho$ deluje na koeficiente racionalnih funkcij, v definicijah $f,g$.
Zato velja
...
&{} = n\sum_i ([T+Q_i]-[Q_i]) \nonumber \\
&{} = \text{div}\left(\prod_i(f_T \circ \tau_{-Q_i})\right). \nonumber
\end{align}
...
\begin{izrek}
\label{izrek:Miller}
Naj bo $E$ eliptična krivulja in naj bosta $P=(x_P,y_P), Q = (x_Q,y_Q)$ točki različni od $\infty$, ki ležita na $E$.
\begin{enumerate}
\item Označimo z $\lambda$ naklon premice, ki povezuje točki $P$ in $Q$. V primeru, da sta točki enaki, naj $\lambda$ predstavlja naklon tangente v točki. Če pa je premica navpična ($x_P = x_Q$), potem privzamemo, da je $\lambda = \infty$. Definirajmo funkcijo $g_{P,Q}$ na sledeči način:
\[ g_{P,Q} =
\begin{cases}
...
saj je točka $\infty$ nevtralni element.
Navpične premice pa sekajo $E$ v neki točki $P$ in $-P$. Torej velja
$$\Div{x-x_{P+Q}} = [P+Q] + [-P-Q] - 2[\infty].$$
Od tod sledi, da ima funkcija
$$g_{P,Q} =\frac{y-\lambda x - \mu}{x-x_{P+Q}}$$
željen delitelj
$$\Div{g_{P,Q}} = [P] + [Q] - [P+Q] - [\infty].$$
Z uporabo formul za seštevanje točk~\ref{sestevanje}, dobimo $x_{P+Q} = \lambda^2-x_P-x_Q$. Prav tako pa lahko izrazimo $\mu$ kot $\mu = y_P-\lambda x_P$. Od tod dobimo
$$g_{P,Q} = \frac{y-y_P-\lambda(x-x_P)}{x+x_P+x_Q-\lambda^2} .$$

V primeru, da velja $\lambda = \infty$, potem je $P+Q = \infty$. V tem primeru želimo dobiti delitelj oblike $\Div{g_{P,Q}} = [P] + [-P] - 2[\infty]$. Tak delitelj pa ima ravno funkcija $x-x_P$.

\item Dokažemo s pomočjo indukcije, pri čemer upoštevamo $\Div{g_{T,T}} = 2[T]-[2T]-[\infty]$ in $\Div{g_{T,P}} = [T]+[P]-[T+P]-[\infty]$.
Preverimo algoritem na primeru $m = 3$. Binarni zapis $m$ je $11$. Sledimo korakom algoritma~\ref{alg:Miller},
...
\begin{primer}
Naj bo $y^2 = x^3+30x+34$ eliptična krivulja nad poljem $\F_{631}$. Izberimo točki $P = (36,60)$ in $Q = (121,387)$. Izračunati želimo Weilovo parjenje $e_5(P,Q)$

Če želimo uporabiti Millerjev algoritem, moramo izbrati točko $S$ iz izreka~\ref{trd-WeilPar}. Izberimo $S=(0,36)$. Hitro lahko preverimo, da $S$ res zadošča kriterijem izbire.
Za izračun $e_5(P,Q)$ moramo izračunati $f_P,f_Q$ v dveh različnih točkah. Poglejmo si podrobnejši izračun za $f_P(S)$.

V prvem koraku, moramo izračunati $g_{P,P}(S)$. Po vseh potrebnih izračunih dobimo $$\text{naklon} = 569,\ \text{števec} = 268,\ \text{imenovalec} = 14.$$ Od tod sledi $g_{P,P}(S) = 560$. Ker velja $m_1 = 0$, po prvem koraku dobimo $f = 560$ in $T = 2P$. V drugem koraku poračunamo $g_{2P,2P}(S)$, kjer kot rezultat dobimo $399$. Vrednost $f$ pa moramo popraviti na $362$. V tem koraku namreč velja $m_0 = 1$, torej moramo poračunati še
$g_{4P,P}(S)$. Premica med $P$ in $4P$ je navpična, zato uporabimo drugo vejo funkcije $g$. Kot rezultat dobimo $g_{4P,P}(S) = 595$. Še zadnjič popravimo $f$ in kot končni rezultat dobimo
$f_P(S) = 219$.
...
\item Alenka in Boris se (javno) dogovorita za eliptično krivuljo $E$ nad končnim obsegom $\Fq{q}$, ter za točko $P \in \E{\Fq{q}}$.
...
Kot sam ključ bi lahko na koncu Alenka in Boris uporabila npr.\  zadnjih $256$ bitov x-koordinate točke $abP$. Tu se zanašamo na to, da je iz $E, \F{q},P, P_a, P_b$ težko izračunati $baP$, kar pa je odvisno od same izbire krivulje E.

To nas privede do tako imenovanega problema diskretnega logaritma v grupi točk nad eliptičnimi krivuljami.
Definirajmo najprej problem diskretnega logaritma v splošni grupi $G$.
\begin{definicija}
...
V našem primeru, kjer je imamo grupo točk na neki krivulji $E$, lahko problem diskretnega logaritma zapišemo kot:

Naj bosta $P, Q \in E$. Iščemo tak $k$, da bo veljalo $kP = Q$.


\begin{trditev}
Če znamo rešiti problem diskretnega logaritma, potem smo rešili tudi problem Diffie-Hellmanove izmenjave ključev. Povedano drugače velja
...
Poleg klasičnega problema diskretnega logaritma lahko formuliramo tudi odločitveni problem diskretnega logaritma. Ta je, ali znamo pri danih podatkih $P,aP,bP,Q$ in $\E{F_q}$ preveriti, če velja $Q = abP$.
...
\begin{primer}
Naj bo $E$ podana z enačbo $y^2=x^3+1$ nad $\F_q$, kjer je $q \equiv 2 \MOD{3}$. Naj bo $\omega \in \F_{q^2}$ primitivni tretji koren enote. Opazimo, da $\omega \notin \F_q$, saj je red $\F^{\times}_q$ enak $q-1$, kar pa ni deljivo s tri.
...
Predpostavimo sedaj, da poznamo $P,aP,bP,Q$ in želimo preveriti ali velja $Q=abP$. Najprej preverimo, če je $Q$ večkratnik $P$. Po trditvi~\ref{trd:5.1} bo to res natanko tedaj, ko je $e_n(P,Q) = 1$.
...
\begin{lema}
\label{lema:6.1}
Naj bo $n \in \N$ število, ki ni deljivo s 3.
Če ima $P \in \E{\F_q}$ red n, potem je $e_n'(P,P)$ n-ti koren enote.
\end{lema}
...
Ločimo dva primera.
\begin{itemize}
\item Če je $vP = \infty$, potem iz definicije $\beta$ sledi $uP = \infty$. To pa pomeni, da je $u \equiv 0 \MOD{n}$.
\item Če velja $vP \neq \infty$, potem zapišimo $vP = (x,y)$, kjer sta $x,y \in \F_q$. Dobimo
$$(\omega x,y) = \beta(vP) \in \E{\F_q}.$$
Ker $\omega \notin \F_q$, mora veljati $x = 0$. Torej je točka $vP$ oblike $(0,\pm 1)$. Take točke pa imajo red tri, kar lahko izpeljemo iz definicije grupe. To pa ni možno, saj smo predpostavili, da $3 \nmid n$.
\end{itemize}
...
Če smo enačbo primerno izbrali, potem bo imel sistem enolično rešitev modulo $p-1$. To pa ravno pomeni, da smo našli diskretne logaritme baze. Vprašanje je torej še, kako generiramo te enačbe. Eden iz med načinov je, da izberemo naključno število $x$, ter poračunamo $\alpha^x \MOD{p}$. Za tem pa preverimo, če ima dobljeno število vse faktorje v bazi.

Zaradi drugega koraka v algoritmu, lahko uvrstimo izračun indeksa pod algoritme tipa Las Vegas, saj naključno izberemo število $1\leq s \leq p-2$, ter izračunamo $\gamma \equiv \beta \alpha^s \MOD{p}$. V tej enačbi je $\beta$ število katerega diskretni logaritem nas zanima. Sedaj pa $\gamma$ poskusimo zapisati kot produkt faktorjev iz baze. Denimo, da lahko zapišemo
...
$$3^x \equiv \pm \text{produkt praštevil iz baze} \mod 1217.$$
...
Če enačbe sedaj zapišemo z uporabo logaritmom, dobimo:
\begin{align}
...
Od tod dobimo $L(2) = 216$, $L(11)=1059$, $L(7) = 113$, $L(5) = 819$, $L(13) = 87$ in $L(3)=1$.
Sedaj poračunamo še $3^j\cdot 37$ za različne $j$, dokler ne dobimo $3^j\cdot 37 \equiv \text{produkt elementov iz baze}$.
...
\section{MOV}
MOV napad, poimenovan po njegovih avtorjih Menezes, Okamoto in Vanstone, s pomočjo Weilovega parjenja pretvori problem diskretnega logaritma iz $E(\F_{q})$ v problem diskretnega logaritma nad $\F^{\times}_{q^m}$~\cite{Washington2008}. Na ta način se izognemo računanju nad krivuljo. Nov problem diskretnega logaritma pa lahko rešimo z različnimi napadi, med drugim tudi z napadom izračun indeksa~\ref{IndexCalc} \cite{Washington2008}. MOV napad deluje, če velikost polja $\F_{q^m}$ ni dosti večja od velikosti polja $\F_{q}$. Postopek napada sledi poteku dokaza naslednje trditve.

\begin{trditev}
\label{trd:5.1}
Naj bo $E$ eliptična krivulja nad $\F_{q}$. Izberimo točki $P,Q \in E(\F_{q})$, kjer ima $P$ red $N$. Predpostavimo, da velja $\text{gcd}(N,q)=1$. Potem obstaja tako število $k$, da velja $Q = kP$ natanko tedaj, ko je $NQ = \infty$ in $e_N(P,Q)=1$.
\end{trditev}

\begin{proof}
$(\Rightarrow)$ Če je $Q = kP$, potem je $NQ = kNP$. Ampak, ker je red $P$ enak $N$, od tod sledi $kNP = \infty$. Prav tako je
...
$E[N] \cong \Z_N \oplus \Z_n$. Sedaj izberemo točko $R$ tako, da je $\{P,R \}$ baza $E[N]$. Ker $P$ in $R$ tvorita bazo, lahko $Q$ zapišemo kot
$$Q = aP+bR,$$
za neki števili $a,b \in \N$. Po posledici definicije Weilovega parjenja~\ref{PosledicaTrdParj} velja, da je 

\noindent $e_N(P,R)=\zeta$ generator $\mu_N$.
Po predpostavki je $e_N(P,Q) = 1$, torej
...
Ker imajo vse točke $E[N]$ koordinate v $\overline{\Fq{q}} = \cup_{j\geq 1}\Fq{q^j}$ tak $m$ obstaja. Prav tako je $\mu_N$ v $\Fq{q^m}$.
...
$$k > log^2(p),$$
kjer MOV napad krivuljo $E(\F_p)$ pretvori v grupo $\F^{\times}_{p^k}$.

MOV napad deluje na supersingularnih krivuljah, saj za take krivulje običajno lahko vzamemo $m=2$.

\begin{definicija}
Eliptična krivulja $E$ v polju s karakteristiko $p$ je \emph{supersingularna}, če je $E[p] \cong 0$.
\end{definicija}
...
\begin{proof}
Zapišimo $X^2-aX+q = (X-\alpha)(X-\beta)$. Trditev~\ref{trd:4.12} pravi, da velja
$$\# \E{\Fq{q^n}}  = q^n+1-(\alpha^n+\beta^n). $$

Po lemi~\ref{lema:4.13} za $s_n = \alpha^n + \beta^n$ velja rekurzivna zveza
...
To pa ravno pomeni, da v $\E{\Fq{q^n}} $ ni točk reda $p$ za noben $n\geq 1$. Ker lahko zapišemo $\overline{\Fq{q}}$ kot $\bigcup_{n \geq 1} \Fq{q^n}$, tudi v $\overline{\Fq{q}}$ nimamo nobene točke reda $p$. To pa po definiciji supersingularnosti pomeni, da je $E$ supersingularna.
...
Mali Fermatov izrek nam pove $a^{p-1} \equiv 1 \MOD{p}$. Torej ima $E(\Fq{q^{p-1}})$ red deljiv s $p$ in zato vsebuje element reda $p$. To pa ravno pomeni, da $E$ ni supersingularna.

Zadnji del trditve pa sledi iz dejstva, da je
$$\# \E{\Fq{q}}  = q+1-a \equiv 1-a \MOD{p}.$$
Od tod vidimo, da je $\# \E{\Fq{q}} \equiv 1 \MOD{p}$ natanko tedaj, ko je $a\equiv 0 \MOD{p}$.
\end{proof}

\begin{opomba}
Izkaže se, da je pri pogoju $q=p \geq 5$ definicija superingularnosti ekvivalentna temu, da je $a=0$.
\end{opomba}
Naslednja trditev bo utemeljila izbiro $m$ pri supersingularnih krivuljah.

\begin{trditev}
Naj bo $E$ eliptična krivulja nad $\F_q$, ter naj velja $a = q+1-\#\E{\F_q} =0$. Izberimo $N\in \N$. Če obstaja točka $P \in \E{\F_q}$ reda $N$, potem velja
$$E[N] \subseteq \E{\F_{q^2}}.$$
\end{trditev}

\begin{proof}
 Frobeniusev endomorfizem $\phi_q$ po izreku~\ref{izrek:4.10} zadošča enačbi $\phi^2_q-a\phi_q+q=0$. Ker velja $a = 0$, to pomeni
$$\phi^2_q = -q.$$

Naj bo $S \in E[N]$. Ker velja $\#\E{\F_q} = q+1$ in ker obstaja točka reda $N$, od tod izhaja da $N|q+1$. Povedano drugače, $-q \equiv 1 \MOD{N}$. Od tod sledi
$$\phi^2_q(S) = -qS = 1\cdot S.$$
Po lemi~\ref{lema:4.5} je torej $S \in \E{\F_{q^2}}$. 
\end{proof}

\begin{primer}
Denimo, da želimo rešiti problem diskretnega logaritma na krivulji $E: \ y^2 = x^3+x \MOD{\Fq{547}}$, podanega s točkama $P=(67,481),Q = (167,405)$. Iščemo tak $k$, da bo veljalo $P = kQ$.

Najprej moramo prvotno polje razširiti z ustreznim $m$. Ker je podana krivulja supersingularna, lahko za $m$ izberemo $2$. Krivuljo torej razširimo na polje $\F_{547^2}$. Za nerazcepni polinom stopnje $2$ vzemimo polinom $x^2+543x+2$. Elementi tega polja so torej polinomi stopnje $1$ s koeficienti v $\F_p$.
V naslednjem koraku izberemo neko naključno točko $T$. Denimo, da smo izbrali točko $T = (24x+219,273x+466)$. Z algoritmom~\ref{alg:babyGiant} sedaj izračunamo red $M$ točke $T$, in dobimo $M = 274$. Izračunati moramo tudi red $N$ točke $P$, le ta pa je enak $N = 137$. Največji skupni delitelj števil $M$ in $N$ je $137$. Zato dobimo novo točko $T_1$ kot 
$$T_1 = (M/\text{gcd}(M, N)) T = (274/137)T = 2T = (440x+318,363x+296).$$
Sedaj s pomočjo Weilovega parjenja poračunamo
$$e_{137}(P,T_1) = 50x+422, \ e_{137}(Q,T_1) = 416x+519.$$
Na tem mestu moramo rešiti problem diskretnega logaritma v polju $\F_{547^2}$. To lahko naredimo s posplošitvijo algoritma izračun indeksa na končna polja~\cite{Henri2005}. Rešitev je $83$, kar lahko preverimo.
Če poračunamo
$$(50x+422)^{83} \div (x^2+543x+2),$$
kot ostanek v $\F_{547^2}$ res dobimo polinom $416x+519$.
Postopek z naključno izbranimi $T$ bi morali še nekajkrat ponoviti. Od tod pa bi potem lahko zaključili, da je iskani $k=83$. Ta rezultat lahko s pomočjo algoritma za seštevanje točk tudi hitro preverimo.
\end{primer}

\subsection{Mali korak, Velik korak}
V tem razdelku bomo opisali, kako izračunamo red točke na ekonomičen način. Trenutno je najhitrejši algoritem za izračun reda točke Schoof–Elkies–Atkinsonov algoritem (SEA)~\cite{Schoof1995}. Tu pa si bomo ogledali nekoliko preprostejši algoritem, ki vseeno predstavlja veliko izboljšanje glede na naiven pristop seštevanja točke same s sebo.
Naj bo $P \in E(\F_q)$. Želimo izračunati red točke $P$. Iščemo torej tako število $k$, da bo veljalo $kP = \infty$. Algoritem Mali korak, Velik korak lahko izračuna red točke v približno $4q^{\frac{1}{4}}$ korakih. Koraki algoritma so sledeči:
...
\item Za $k = -m,-m+1,\ldots,m-1,m$ izračunaj točke $Q +k(2mP)$. V primeru, da se kakšna od teh točk ujema s $\pm jP$, prekini računanje in si zapomni ustrezna $k,j$.
\item Izračunaj $(q+1+2mk \mp j)P$ in poglej v katerem primeru je to enako $\infty$. V tem primeru naj bo $M = (q+1+2mk \mp j)$.
\item Faktoriziraj $M$. Označi faktorje števila $M$ s $p_1,\ldots,p_r$.
...
Algoritem~\ref{alg:babyGiant} lahko preoblikujemo do te mere, da namesto reda točke izračunamo število točk na krivulji. Za to moramo ponavljati korake $(1)$-$(6)$ za naključno izbrane točke $P \in E(\Fq{q})$. Ustavimo se, ko najmanjši skupni večkratnik redov točk deli eno samo število $N$ v območju $q+1-2\sqrt{q} \leq N \leq q+q+2\sqrt{q}$. To število $N$ je potem število točk na dani krivulji. Pri tem postopku uporabimo Hassejev izrek o številu točk na eliptični krivulji.
...
$$Q+k(2mP) = Q =  \infty = jP.$$
Za število $M$ prav tako v obeh primerih dobimo isti rezultat, in sicer $M = 548$. Velja $548=2\cdot 2\cdot 137$. Pri izvajanju nadaljnih korakov algoritma število $M$ dvakrat delimo z $2$, saj velja $274P = \infty$ in $137P = \infty$. Ker pa je $137$ praštevilo in $P \neq \infty$, od tod sledi, da je red $N$ točke $P$ je enak $137$.
...
$$\#\E{\F_q} = q.$$
Takim krivuljam rečemo \emph{anomalne krivulje}. V primeru, ko je $p$ praštevilo, je $E[p]$ ciklična grupa, kar pomeni, da bo Weilovo parjenje konstantno enako $1$. Vendar pa za tak tip krivulj obstaja napad, ki deluje še hitreje kot MOV.
...
\begin{proof}
Izberi števili $x_1$ in $x_2$, ki v $\MOD{p}$ podajata x-koordinati $P$ in $Q$. Predpostavimo najprej, da $x_1 \not \equiv x_2 \MOD{p}$. Nato izberimo $y_1$ tako, da velja
...
Na ta način dobimo dve enačbi s pomočjo katerih določimo konstanti $a',b'$.
Enačbi sta
\begin{align}
y_1^2&{}=x_1^3+a'x_1+b', \nonumber  \\
y_2^2&{}=x_2^3+a'x_2+b'. \nonumber
...
Ker je $y_2^2-y_1^2$ po konstrukciji deljivo z $x_2-x_1$ in ker so vse ostale konstante cela števila, sta tudi $a',b' \in \Z$. Točki $P',Q'$ pa ležita na $E'$ po konstrukciji.

Obravnavati moramo še primer, če je $x_1 \equiv x_2 \MOD{p}$. V tem primeru velja $P = \pm Q$. Izberimo potem $x_1 = x_2$, ter $y_1$ tako, da nam $y_1 \MOD{p}$ podaja y-koordinato točke $P$. Na enak način izberemo še $a'$, ter določimo $b' = y_1^2-x_1^3-a'x_1$. $Q'$ pa izberemo kot $\pm P'$.
...
Velja torej $4a'^3+27b'^2 \neq 0$, zato je $E'$ res gladka eliptična krivulja.
\end{proof}


\begin{definicija}
Naj bosta $a,b$ tuji celi števili, za kateri velja $a/b \neq 0$. Zapišimo $a/b = p^ra_1/b_1$, kjer $p \nmid a_1b_1$. Potem je \emph{p-adična vrednost} definirana
kot
$$v_p(a/b) = r.$$
Definirajmo še
$$v_p(0) = \infty.$$
...
\item Če je $(x,y) \in E'(\Q)$, potem je $v_p(x)<0$ natanko tedaj, ko je $v_p(y) <0$. V tem primeru obstaja število $r \geq 1$, za katero velja $v_p(x) = -2r$ in $v_p(y) = -3r$.
\item Preslikava
\begin{align}
...
\item Če je $(x,y) \in E_r'$ in hkrati $(x,y) \not \in E_{r+1}'$, potem $\lambda_r(x,y) \not \equiv 0 \MOD{p}$.
...
Vrnimo se sedaj k prvotnemu problemu. Imamo torej anomalno krivuljo $E$ nad poljem $\F_p$. Želimo poiskati $k$, tako da bo veljalo $Q = kP$. Sledeči algoritem nam vrne rešitev tega problema.
...
\item Če je $P_1' \in E_2'$, izberi nove $E',P',Q'$ in se vrni na korak $2$. V nasprotnem primeru je $l_1 = \lambda_1(P_1'),l_2 = \lambda_1(Q_1')$. Iskani $k$ pa je $k \equiv l_1/l_2 \MOD{p}$.
\end{enumerate}
...
Tu je potrebno podati še utemeljitev zakaj tak napad res deluje. Označimo $K' = kP'-Q'$.
Potem velja
$$\infty = kP-Q = \text{red}_p(kP'-Q') = red_p(K').$$
To pomeni, da je $K' \in E_1'$. Od tod pa sledi, da je $\lambda_1(K')$ definiran in velja
$$\lambda_1(pK') = p \lambda_1(K') \equiv 0 \MOD{p}.$$

Torej velja
$$kl_1-l_2 = \lambda_1(kP_1'-Q_1') = \lambda_1(kpP'-pQ') = \lambda_1(pK') \equiv 0 \MOD{p}.$$
To pa je ravno to, kar smo želeli dokazati.
Izkaže se, da je ta algoritem nepraktičen, saj ima x-koordinata točk ponavadi približno $p^2$ števk.
Vendar pa se lahko problemu izognemo tako, da delamo po modulu $p^2$ namesto v $\Q$.
...
\begin{enumerate}
\item Alenka predstavi sporočilo kot neko točko $M\in \E{\F_q}$.
\item Alenka naključno izbere skrivno število $k$ in izračuna $M_1 = kP$.
\item Alenka izračunaj $M_2 = M+kB$.
\item Alenka pošlje Borisu $M_1,M_2$.
\end{enumerate}

Boris izračuna $M$ kot
$$M = M_2-sM_1.$$
Prepričajmo se, da Boris na ta način res dobi točko $M$:
$$M_2-sM_1 = (M+kB)-k(sP)=M+ksP-ksP = M.$$
...
Pomembno pri celotni shemi pa je tudi to, da Alenka za različna sporočila ne uporablja istega števila $k$. Recimo, da bi za sporočili $M,M'$ Alenka uporabila isti $k$. Vsak tretji poslušalec bi lahko opazil, da sta v tem primeru točki $M_1,M_1'$ enaki, in zato bi lahko izračunal
$$M_2-M_2' = M'-M.$$
...
Vprašanje, ki ga moramo še razrešiti je, kako sporočilo predstaviti kot točko na krivulji. Ena izmed možnosti je način, ki ga je predlagal Koblitz~\cite{Koblitz1987}. Denimo, da je krivulja podana z enačbo
...
V prejšnjih poglavjih smo videli, kako lahko s pomočjo Weilovega parjenja izvedemo napad na problem diskretnega logaritma nad eliptičnimi krivuljami. Supersingularne krivulje so med tistimi, na katerih ta napad deluje. Sedaj pa si bomo ogledali še kriptosistem, ki uporablja supersingularne krivulje~\cite{Boneh2003}. Tu se opremo na dejstvo, da kljub napadom kot je MOV, problem diskretnega logaritma v $\F^{\times}_p$ ni enostaven. Če izberemo dovolj velik $p$, je ta problem še vedno zelo zahteven. Supersingularne krivulje izbiramo zato, ker lahko izkoristimo njihove posebne lastnosti.

Obravnavajmo krivuljo $E$ podano z enačbo $y^2=x^3+1$ nad $\F_p$, kjer je $p \equiv 2 \MOD{3}$. Take krivulje so supersingularne. Naj bo $\omega \in \F_{p^2}$ primitivni tretji koren enote. Definirajmo preslikavo
\begin{align}
&{}\beta:\E{\F_{p^2}} \rightarrow \E{\F_{p^2}},\nonumber \\
&{}(x,y) \mapsto (\omega x,y), \ \beta(\infty) = \infty. \nonumber
\end{align}
...
Že v lemi~\ref{lema:6.1} smo pokazali, da v primeru ko $3 \nmid n$ in ima $P \in \E{\F_p}$ red $n$, je $e'_n(P,P)$ primitivni n-ti koren enote.

Ker je $E$ supersingularna, ima red $p+1$. Predpostavimo še, da za neko praštevilo $l$ velja $p = 6l-1$. To pomeni, da ima točka $6P$ red $l$ ali $1$ za vsako točko $P$.
...
\begin{itemize}
\item S pomočjo privatnega ključa $D_I$ izračuna $h_I = e'_l(D_I,u)$.
\item Izračuna $m= v \oplus H_2(h_I)$.
\end{itemize}
